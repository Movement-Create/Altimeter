...
class AgentRunner {
    constructor(outputChannel) {
        ...
        this._config = {};
        this._bravePrompted = false;
        this._lastSearchedPaths = [];
    ...
    getConfig() {
        ...
        return {
            ...
            maxBudget: config.get('maxBudget', 1.0),
            ...
            braveSearchKey: config.get('braveSearchKey', ''),
            altimeterPath: config.get('altimeterPath', ''),
        };
    }
    getAltimeterPath() {
    ...
    async ensureApiKeys(provider) {
        ...
        const braveExisting = cfg.get('braveSearchKey', '') || process.env.BRAVE_API_KEY || '';
    ...
    getPermissionRules() {
        const config = vscode.workspace.getConfiguration('altimeter');
        return config.get('permissionRules', {
            file_read: 'always',
            grep: 'always',
            glob: 'always',
            file_write: 'ask',
        ...
        return new Promise((resolve, reject) => {
            ...
            proc.on('close', (code) => {
                ...
                if (code !== 0) {
                    ...
                    reject(new Error(`Altimeter exited with code ${code}. Stderr: ${stderrSnippet}${hint}\n\nSee full details: View → Output → Altimeter`));
                    return;
                }
    ...
    _parseRunOutput(stdout) {
        ...
        try {
            const result = JSON.parse(stdout.slice(braceStart, braceEnd + 1));
            if (result && typeof result.text === 'string') {
                return result;
            }
        }
    ...
    async listTools() {
        ...
        return new Promise((resolve, reject) => {
            let stdout = '';
            const proc = (0, child_process_1.spawn)('node', [indexPath, 'tools'], {
                cwd: altimeterDir,
                env,
            });
    ...
    async listSessions() {
        ...
        return new Promise((resolve, reject) => {
            let stdout = '';
            const proc = (0, child_process_1.spawn)('node', [indexPath, 'session', 'list'], {
                cwd: altimeterDir,
                env,
            });
            proc.stdout.on('data', (d) => (stdout += d.toString()));
    ...
    async addMemory(fact) {
        ...
        return new Promise((resolve, reject) => {
            let stdout = '';
            const proc = (0, child_process_1.spawn)('node', [indexPath, 'memory', 'add', fact], {
                cwd: altimeterDir,
                env,
            });
            proc.stdout.on('data', (d) => (stdout += d.toString()));
    ...
    async searchMemory(query) {
        ...
        return new Promise((resolve, reject) => {
            let stdout = '';
            const proc = (0, child_process_1.spawn)('node', [indexPath, 'memory', 'search', query], {
                cwd: altimeterDir,
                env,
            });
            proc.stdout.on('data', (d) => (stdout += d.toString()));
